...
import { APIPromise } from 'sambanovasdk/core/api-promise';

import util from 'node:util';
import Sambanova from 'sambanovasdk';
import { APIUserAbortError } from 'sambanovasdk';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['SAMBANOVA_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Sambanova) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Sambanova({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Sambanova({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['SAMBANOVA_LOG'] = 'debug';
      const client = new Sambanova({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['SAMBANOVA_LOG'] = 'not a log level';
      const client = new Sambanova({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'SAMBANOVA_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['SAMBANOVA_LOG'] = 'debug';
      const client = new Sambanova({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    ...
    test('empty env variable', () => {
      ...
      const client = new Sambanova({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.sambanova.ai/v1');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new Sambanova({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['SAMBANOVA_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Sambanova({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['SNOVA_API_KEY'] = 'My API Key';
    const client = new Sambanova();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['SNOVA_API_KEY'] = 'another My API Key';
    const client = new Sambanova({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});

...
import {
  Audio,
  AudioReasoningParams,
  AudioReasoningResponse,
  AudioTranscribeParams,
  AudioTranscribeResponse,
  AudioTranslateParams,
  AudioTranslateResponse,
} from './resources/audio';
import { Chat, ChatGenerateCompletionParams, ChatGenerateCompletionResponse } from './resources/chat';
import { Embedding, EmbeddingGenerateParams, EmbeddingGenerateResponse } from './resources/embedding';
...
export interface ClientOptions {
  /**
   * Defaults to process.env['SNOVA_API_KEY'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['SAMBANOVA_LOG'] or 'warn' if it isn't set.
   */
...
export class Sambanova {
   ...
   * API Client for interfacing with the Sambanova API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['SNOVA_API_KEY'] ?? null]
   * @param {string} [opts.baseURL=process.env['SAMBANOVA_BASE_URL'] ?? https://api.sambanova.ai/v1] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('SAMBANOVA_BASE_URL'),
    apiKey = readEnv('SNOVA_API_KEY') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? Sambanova.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('SAMBANOVA_LOG'), "process.env['SAMBANOVA_LOG']", this) ??
      defaultLogLevel;
  ...
  embedding: API.Embedding = new API.Embedding(this);
  models: API.Models = new API.Models(this);
}
Sambanova.Chat = Chat;
Sambanova.Audio = Audio;
Sambanova.Embedding = Embedding;
